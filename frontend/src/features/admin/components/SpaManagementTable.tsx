import { Building2, Clock, Eye, Pencil } from "lucide-react";
        ...
        {spas.map((spa) => (
          <div key={spa.id} className={`grid min-w-[860px] grid-cols-[1.4fr_1fr_0.8fr_0.8fr_1.2fr] items-center gap-md px-lg py-md ${selectedSpaId === spa.id ? "bg-soft-mint/50" : "bg-white"}`}>
            ...
            <div className="flex justify-end gap-xs">
              <Link to={`/admin/spas/${spa.id}`} className="inline-flex h-9 items-center gap-1 rounded-full border border-botanical-border bg-white px-md text-label-caption font-black uppercase tracking-[0.1em] text-sage-secondary transition-colors hover:border-primary hover:bg-soft-mint">
