...
export function SpaManagementTable({ spas, selectedSpaId, onSelect, onHide, onDelete }: { spas: Spa[]; selectedSpaId?: string; onSelect: (spaId: string) => void; onHide: (spaId: string, status: Spa["status"]) => void; onDelete: (spaId: string) => void }) {
  return (
    <AdminTableShell>
      <div className="grid min-w-[860px] grid-cols-[1.4fr_1fr_0.8fr_0.8fr_1fr] gap-md border-b border-botanical-border bg-soft-mint/60 px-lg py-md text-label-caption font-black uppercase tracking-[0.16em] text-sage-secondary">
      ...
      <div className="divide-y divide-botanical-border overflow-x-auto">
        {spas.map((spa) => (
          <div key={spa.id} className={`grid min-w-[860px] grid-cols-[1.4fr_1fr_0.8fr_0.8fr_1fr] items-center gap-md px-lg py-md ${selectedSpaId === spa.id ? "bg-soft-mint/50" : "bg-white"}`}>
